...
function verifyLencoSignature(payload: string, signature: string, secret: string): boolean {
  if (!secret) {
    // This is a critical security check. If the secret is not configured,
    // we must reject the webhook to prevent attackers from spoofing events.
...
serve(async (req) => {
  ...
  try {
    ...
    if (!verifyLencoSignature(rawBody, signature, webhookSecret)) {
      console.error("Invalid webhook signature. The webhook secret might be missing or the signature is incorrect.");

...
serve(async (req) => {
  ...
  try {
    ...
    // 🛡️ Security: Enforce signature verification using the shared module.
    // This is a critical security measure to prevent webhook spoofing.
    try {
      if (!verifyWebhookSignature(rawBody, signature, webhookSecret, "sha256")) {
      ...
      return new Response(JSON.stringify({ error: error.message }), {
        status: 400, // Bad Request, as the secret might be missing
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

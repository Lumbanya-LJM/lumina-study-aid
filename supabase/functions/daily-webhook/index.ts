...
serve(async (req) => {
  ...
  try {
    ...
    // 🛡️ Security: Verify the webhook signature to prevent spoofing.
    const rawBody = await req.text();
    const signature = req.headers.get("Authorization") || "";

    try {
      // Daily.co webhooks use HMAC-SHA1 for signature verification.
      if (!verifyWebhookSignature(rawBody, signature, DAILY_WEBHOOK_SECRET, "sha1")) {
        console.error("Invalid webhook signature for Daily.co");
      ...
      console.error("Daily webhook signature verification error:", error.message);
